...
callback:function(){},updateCallback:function(){},removeDateText:!0,displayElement:null},c),f=b(this),k=f.find(a.element).first(),l="object"==typeof console,G=a.displayElement?f.find(a.displayElement):f;if(1>k.length)return l&&console.log("[Erro] Elemento com o nome da promo\u00e7\u00e3o n\u00e3o encontrado \n ("+k.selector+")"),f;c=k.text()||"";if(0>c.indexOf(a.separator))return l&&console.log("[Erro] Separador \u201c"+a.separator+"\u201d n\u00e3o encontrado."),f;var t=c.split(a.separator).pop().trim(),
...
"").replace(t,""))},getTimeRemaining:function(){h.setMilliseconds(h.getMilliseconds()+a.updatInterval);var d=countdown(h,E,countdown.DAYS|countdown.HOURS|countdown.MINUTES|countdown.SECONDS);m=d.days;n=d.hours;u=d.minutes;v=d.seconds;2===a.outputFormat&&(n=24*m+n,m="");e.updateHtml()},toDate:function(d){var b={jan:0,fev:1,mar:2,abr:3,mai:4,jun:5,jul:6,ago:7,set:8,out:9,nov:10,dez:11},a=d.replace(/[a-z]{3}/,function(a){return b[a]||""}).replace(",","").split(" "),c=a[3].split(":");h=new Date(a[2],
a[0],a[1],c[0],c[1],c[2]);isNaN(h.getTime())&&(l&&console.log("Erro ao processar a data retornada via Ajax \n \u201c"+d+"\u201d"),h=new Date)},updateHtml:function(){s.text(v);r.text(u);q.text(n);p.text(m);1==v?(z.hide(),D.show()):(D.hide(),z.show());1==u?(y.hide(),C.show()):(C.hide(),y.show());1==n?(x.hide(),B.show()):(B.hide(),x.show());1==m?(w.hide(),A.show()):(A.hide(),w.show());0==F?(f.removeClass("vtex-cd_loading"),e.updateCounter(),a.callback()):a.updateCallback();F++},formatHtml:function(){var d=
a.htmlFormat.replace("%days%",'<span class="vtex-cd_days"></span>').replace("%hours%",'<span class="vtex-cd_hours"></span>').replace("%minutes%",'<span class="vtex-cd_minutes"></span>').replace("%seconds%",'<span class="vtex-cd_seconds"></span>'),d=b(d);p=d.find(".vtex-cd_days");q=d.find(".vtex-cd_hours");r=d.find(".vtex-cd_minutes");s=d.find(".vtex-cd_seconds");w=p.siblings(".vtex-cd_p");x=q.siblings(".vtex-cd_p");y=r.siblings(".vtex-cd_p");z=s.siblings(".vtex-cd_p");A=p.siblings(".vtex-cd_s");B=
q.siblings(".vtex-cd_s");C=r.siblings(".vtex-cd_s");D=s.siblings(".vtex-cd_s");G.addClass("vtex-cd_loading").append(d)},updateCounter:function(){setInterval(e.getTimeRemaining,a.updatInterval)}};b.ajax({url:"/no-cache/HoraAtualServidor.aspx",data:"html",success:function(a,b,c){e.toDate(a.toLowerCase().trim());e.getTimeRemaining()},error:function(a,b,c){l&&console.log("Erro na requisi\u00e7\u00e3o Ajax");e.getTimeRemaining()}});e.removeDateText();e.formatHtml()})})(jQuery);